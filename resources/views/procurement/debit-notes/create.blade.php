...
@section('content')
    <div class="max-w-3xl mx-auto">
        ...
        <div class="flex items-center gap-4 mb-6">
            ...
            <div>
                <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Create Debit Note</h1>
                <p class="text-gray-500 dark:text-gray-400">Price adjustment for GRR: {{ $goodsReturnRequest->grr_number }}
                </p>
            </div>
        </div>
                        ...
                        <div class="flex justify-between items-start">
                            <div>
                                ...
                                <p class="text-sm text-gray-500 dark:text-gray-400">
                                    {{ $goodsReturnRequest->quantity_affected }} units @ Rp
                                    {{ number_format($goodsReturnRequest->goodsReceiptItem->purchaseOrderItem->unit_price, 0, ',', '.') }}
                                </p>
                            </div>
                            <div class="text-right">
                                <p class="text-xs text-gray-500 uppercase">Original Value</p>
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <textarea name="reason" rows="3"
                                ...
                                placeholder="Explain the reason for the price adjustment...">{{ $goodsReturnRequest->issue_description }}</textarea>
                        </div>
                    </div>
                </div>
            </div>
...
@push('scripts')
    <script>
        ...
        function updateSummary(original) {
            ...
            document.getElementById('display_deduction').textContent = '- Rp ' + deduction.toLocaleString('en-US').replace(/,/g, '.');
            document.getElementById('display_final').textContent = 'Rp ' + final.toLocaleString('en-US').replace(/,/g, '.');
        }
    </script>
@endpush