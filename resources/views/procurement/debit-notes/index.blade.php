...
<div class="flex flex-col md:flex-row md:items-center md:justify-between mb-6 gap-4">
    <div>
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white flex items-center gap-2">
            <i data-feather="file-minus" class="w-6 h-6 text-red-500"></i>
            Debit Notes
        </h1>
        <p class="text-gray-500 dark:text-gray-400 mt-1">Daftar penyesuaian harga untuk barang bermasalah</p>
    </div>
</div>
...
@if($debitNotes->isEmpty())
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-12 text-center">
        ...
        <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-2">Belum Ada Debit Note</h3>
        <p class="text-gray-500 dark:text-gray-400">Debit note akan muncul setelah ada penyesuaian harga dari GRR.</p>
    </div>
@else
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            <thead class="bg-gray-50 dark:bg-gray-700/50">
                <tr>
                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">No. DN</th>
                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">PO Number</th>
                    <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Nilai Original</th>
                    <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Potongan</th>
            ...
            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                @foreach($debitNotes as $dn)
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <a href="{{ route('procurement.po.show', $dn->purchaseOrder) }}" class="text-sm text-primary-600 hover:underline dark:text-primary-400">
                        ...
                        <td class="px-6 py-4 text-right">
                            <span class="text-sm font-semibold text-red-600 dark:text-red-400">- {{ $dn->formatted_deduction_amount }}</span>
                            <span class="text-xs text-gray-500 block">{{ $dn->deduction_percentage }}%</span>
                        </td>
                        <td class="px-6 py-4">
                            @if($dn->isApprovedByVendor())
