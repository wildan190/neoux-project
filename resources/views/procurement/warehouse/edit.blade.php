...
<div class="max-w-2xl mx-auto">
    <div class="bg-white dark:bg-gray-800 rounded-3xl shadow-xl border border-gray-100 dark:border-gray-700 overflow-hidden">
        <div class="p-8 border-b border-gray-100 dark:border-gray-700">
            <h1 class="text-2xl font-black text-gray-900 dark:text-white uppercase tracking-tight">Edit Warehouse</h1>
            <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">Modify details for {{ $warehouse->name }}</p>
        </div>

        <form action="{{ route('procurement.warehouse.update', $warehouse) }}" method="POST" class="p-8">
            @csrf
            @method('PUT')
            <div class="space-y-6">
                <div>
                ...
                <div>
                    ...
                    <textarea name="address" id="address" rows="4"
                        class="w-full border-gray-200 dark:border-gray-700 dark:bg-gray-900 dark:text-white rounded-xl shadow-sm focus:border-primary-500 focus:ring-primary-500 text-sm py-3 px-4">{{ old('address', $warehouse->address) }}</textarea>
                    @error('address')
                        <p class="mt-1 text-xs text-red-500 font-bold uppercase tracking-tight">{{ $message }}</p>
                    @enderror
                </div>
