    ...
    <div class="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <div class="flex items-center gap-3 mb-1">
                <span class="px-3 py-1 bg-primary-600 text-white rounded-lg text-[10px] font-black uppercase tracking-widest">{{ $tab === 'active' ? 'Active' : 'Historical' }} Negotiations</span>
                <span class="text-xs font-bold text-gray-400 uppercase tracking-widest">{{ $offers->total() }} Total</span>
            </div>
            <h1 class="text-2xl font-black text-gray-900 dark:text-white uppercase tracking-tight">
                {{ $tab === 'active' ? 'Ongoing Discussions' : 'Negotiation History' }}
            </h1>
        </div>
    </div>
    ...
    @if($offers->isEmpty())
        <div class="bg-white dark:bg-gray-800 rounded-[2.5rem] p-20 text-center border border-gray-100 dark:border-gray-700 shadow-sm">
            <div class="w-20 h-20 bg-gray-50 dark:bg-gray-900 rounded-3xl flex items-center justify-center mx-auto mb-6">
                <i data-feather="{{ $tab === 'active' ? 'message-square' : 'archive' }}" class="w-10 h-10 text-gray-200"></i>
            </div>
            ...
            <p class="text-[11px] font-bold text-gray-500 dark:text-gray-400 mt-2 uppercase tracking-widest px-12 leading-relaxed">
                {{ $tab === 'active' ? 'You currently have no offers in the negotiating stage.' : 'Your historical negotiations will appear here once finalized.' }}
            </p>
        </div>
    @else
                    ...
                    <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-8">
                        <div class="flex-1">
                            <div class="flex items-center gap-3 mb-3">
                                ...
                                <span class="px-2 py-0.5 {{ $currentColor }} text-[8px] font-black uppercase tracking-widest rounded-md ring-1 italic">{{ str_replace('_', ' ', $offer->status) }}</span>
                            </div>
                            ...
                            <div class="bg-gray-50 dark:bg-gray-900 px-4 py-3 rounded-xl border border-gray-100 dark:border-gray-800">
                                <p class="text-[8px] font-black text-gray-400 uppercase tracking-widest mb-1">Last Negotiation Context</p>
                                <p class="text-xs text-gray-700 dark:text-gray-300 italic">"{{ $offer->negotiation_message ?? 'Check offer details for terms.' }}"</p>
                            </div>
                        </div>

                        <div class="flex flex-col lg:items-end gap-3 rounded-2xl min-w-[200px]">
                            <div class="text-right">
                                <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Total Value</p>
                            ...
                            <div class="flex items-center gap-2 mt-2">
                                <a href="{{ route('procurement.offers.show', $offer) }}"
                                   class="w-full text-center px-5 py-2.5 bg-gray-900 text-white dark:bg-white dark:text-gray-900 rounded-xl text-[10px] font-black uppercase tracking-widest shadow-xl shadow-gray-900/10 hover:scale-105 transition-all">
                                    {{ $offer->status === 'negotiating' ? 'Review & Respond' : 'View Resolution' }}
                                </a>
                            </div>
                        </div>
                    </div>
