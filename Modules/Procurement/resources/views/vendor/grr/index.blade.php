@extends('layouts.app', [
    'title' => 'Sales Returns & Discrepancies',
    'breadcrumbs' => [
        ['name' => 'Home', 'url' => url('/')],
...
@section('content')
    <div class="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <div class="flex items-center gap-3 mb-1">
                <span class="px-3 py-1 bg-indigo-600 text-white rounded-lg text-[10px] font-black uppercase tracking-widest">AFTER-SALES SUPPORT</span>
                <span class="text-xs font-bold text-gray-400 uppercase tracking-widest">{{ $grrList->total() }} Technical Claims</span>
            </div>
            <h1 class="text-2xl font-black text-gray-900 dark:text-white uppercase tracking-tight">Incoming <span class="text-primary-600">Return Claims</span></h1>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-3xl border border-gray-100 dark:border-gray-800 p-2 mb-8 flex flex-wrap gap-2 shadow-sm">
        <a href="{{ route('procurement.grr.index', ['filter' => 'all', 'view' => 'vendor']) }}"
        ...
        <a href="{{ route('procurement.grr.index', ['filter' => 'pending', 'view' => 'vendor']) }}"
        ...
        <a href="{{ route('procurement.grr.index', ['filter' => 'in_progress', 'view' => 'vendor']) }}"
            class="px-6 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all
            {{ $filter == 'in_progress' ? 'bg-primary-600 text-white' : 'text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-900' }}">
            In Processing
        </a>
        <a href="{{ route('procurement.grr.index', ['filter' => 'resolved', 'view' => 'vendor']) }}"
            class="px-6 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all
    ...
    @if($grrList->isEmpty())
        <div class="bg-white dark:bg-gray-800 rounded-[2.5rem] p-20 text-center border border-gray-100 dark:border-gray-700 shadow-sm">
            ...
            <h3 class="text-xl font-black text-gray-900 dark:text-white uppercase tracking-tight">Zero Claims</h3>
            <p class="text-[11px] font-bold text-gray-500 dark:text-gray-400 mt-2 uppercase tracking-widest">Congratulations, all your shipped orders have been technicaly verified by your clients.</p>
        </div>
    @else
                    ...
                    <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-8">
                        <div class="flex items-start gap-6">
                            ...
                            <div>
                                ...
                                <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-4">
                                    CLIENT: {{ $grr->goodsReceiptItem->goodsReceipt->purchaseOrder->purchaseRequisition->company->name }} • LODGED {{ $grr->created_at->format('M d, Y') }}
                                </p>
                                
                                <div class="flex items-center gap-6">
                                    <div>
                                        <p class="text-[8px] font-black text-gray-400 uppercase tracking-widest mb-1">Claimed Product</p>
                                        <p class="text-[11px] font-black text-gray-700 dark:text-gray-300 uppercase tracking-tight leading-none">
                        ...
                        <div class="flex flex-col lg:items-end gap-3">
                            <div class="text-right">
                                <p class="text-[8px] font-black text-gray-400 uppercase tracking-widest mb-1">Issue Context</p>
                                <p class="text-base font-black text-red-600 uppercase tracking-tight">{{ $grr->issue_type_label }}</p>
                            </div>
