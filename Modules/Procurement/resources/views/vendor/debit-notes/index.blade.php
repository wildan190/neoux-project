@extends('layouts.app', [
    'title' => 'Issued Financial Adjustments',
    'breadcrumbs' => [
        ['name' => 'Home', 'url' => url('/')],
...
@section('content')
    <div class="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <div class="flex items-center gap-3 mb-1">
                <span class="px-3 py-1 bg-indigo-600 text-white rounded-lg text-[10px] font-black uppercase tracking-widest">FINANCIAL ADJUSTMENTS</span>
                <span class="text-xs font-bold text-gray-400 uppercase tracking-widest">{{ $debitNotes->total() }} Issued Adjustments</span>
            </div>
            <h1 class="text-2xl font-black text-gray-900 dark:text-white uppercase tracking-tight">Sales <span class="text-primary-600">Debit Notes</span></h1>
        </div>
    </div>
    ...
    @if($debitNotes->isEmpty())
        <div class="bg-white dark:bg-gray-800 rounded-[2.5rem] p-20 text-center border border-gray-100 dark:border-gray-700 shadow-sm">
            ...
            <h3 class="text-xl font-black text-gray-900 dark:text-white uppercase tracking-tight">Zero Adjustments</h3>
            <p class="text-[11px] font-bold text-gray-500 dark:text-gray-400 mt-2 uppercase tracking-widest px-12 leading-relaxed">
                You haven't issued any financial adjustments yet. Debit notes are created in response to technical discrepancies reported by clients.
            </p>
        </div>
    @else
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-gray-50/50 dark:bg-gray-900/50 border-b border-gray-100 dark:border-gray-800">
                            <th class="px-8 py-6 text-[9px] font-black text-gray-400 uppercase tracking-[0.2em]">Note Summary</th>
                            <th class="px-8 py-6 text-[9px] font-black text-gray-400 uppercase tracking-[0.2em]">Authorized (PO)</th>
                            <th class="px-8 py-6 text-[9px] font-black text-gray-400 uppercase tracking-[0.2em]">Customer Entity</th>
                            <th class="px-8 py-6 text-[9px] font-black text-gray-400 uppercase tracking-[0.2em]">Credit Value</th>
                            <th class="px-8 py-6 text-[9px] font-black text-gray-400 uppercase tracking-[0.2em]">Response Status</th>
                    ...
                    <tbody class="divide-y divide-gray-50 dark:divide-gray-800">
                        @foreach($debitNotes as $dn)
                            <tr class="group hover:bg-gray-50/50 dark:hover:bg-gray-900/30 transition-colors">
                                ...
                                <td class="px-8 py-8">
                                    <p class="text-[11px] font-black text-gray-700 dark:text-white uppercase tracking-tight leading-none mb-1">{{ $dn->purchaseOrder->po_number }}</p>
                                </td>
                                <td class="px-8 py-8">
                                    <div class="flex items-center gap-3">
                                        <div class="px-2 py-1 bg-gray-100 dark:bg-gray-700 rounded-md text-[9px] font-black uppercase tracking-widest text-gray-500">
                                            {{ substr($dn->purchaseOrder->purchaseRequisition->company->name, 0, 3) }}
                                        </div>
                                        <span class="text-[11px] font-black text-gray-700 dark:text-gray-300 uppercase tracking-tight">{{ $dn->purchaseOrder->purchaseRequisition->company->name }}</span>
                                    </div>
                                </td>
                                <td class="px-8 py-8">
                                    <p class="text-[11px] font-black text-red-600 tracking-tight leading-none mb-1">{{ number_format($dn->deduction_amount, 2) }} Credit</p>
                                    <p class="text-[9px] font-bold text-gray-400 uppercase tracking-widest">{{ $dn->deduction_percentage }}% VALUE REDUCTION</p>
                                </td>
                                <td class="px-8 py-8">
